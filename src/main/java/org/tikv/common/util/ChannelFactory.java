...
public class ChannelFactory implements AutoCloseable {
    ...
    public ManagedChannel getChannel(String addressStr, HostMapping hostMapping) {
        ...
        if (channel != null && !channel.isShutdown() && !channel.isTerminated()) {
            return channel;
        } else if (channel != null && (channel.isShutdown() || channel.isTerminated())) {
            connPool.remove(addressStr);
        }
                ...
                key -> {
                    ...
                    ThreadFactory tfac =
                            new ThreadFactoryBuilder()
                                    .setDaemon(true)
                                    .setThreadFactory(CDCEventThread::new)
                                    .setNameFormat("grpc-boss-event-group-%d")
                                    .build();

                    Class<? extends Channel> channelType;
                    if (Epoll.isAvailable()) {
                        this.bossEventLoopGroup = new EpollEventLoopGroup(1, tfac);
                        channelType = EpollSocketChannel.class;
                    } else {
                        this.bossEventLoopGroup = new NioEventLoopGroup(1, tfac);
                        channelType = NioSocketChannel.class;
                    }

                    builder.eventLoopGroup(bossEventLoopGroup).channelType(channelType);
                    ...
                    if (workerGroup == null) {
                        workerGroup =
                                Executors.newCachedThreadPool(
                                        new ThreadFactoryBuilder()
                                                .setDaemon(true)
                                                .setNameFormat("grpc-worker-group-%d")
                                                .build());
                    }
                    builder.executor(workerGroup);
    ...
    protected static final class CDCEventThread extends FastThreadLocalThread {
        public CDCEventThread(Runnable r) {
            super(r);
        }
    }

    public Executor getWorkerGroup() {
        return workerGroup;
    }

    public MultithreadEventLoopGroup getBossEventLoopGroup() {
